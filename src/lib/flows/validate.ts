    ...
    for (const n of nodes) {
      if (!reached.has(n.node_key)) {
        issues.push({
          ...
          scope: "node",
          node_key: n.node_key,
          message: `O nó "${n.node_key}" não é acessível a partir do nó de entrada.`,
        });
      }
    }
  }
  ...
  if (trigger_type === "keyword") {
    ...
    if (!keywords || keywords.length === 0) {
      issues.push({
        ...
        message: "Gatilhos por palavra-chave precisam de pelo menos uma palavra-chave.",
      });
    } else {
      ...
      if (blanks > 0) {
        issues.push({
          ...
          scope: "trigger",
          field: "trigger_config.keywords",
          message: `${blanks} palavra-chave${blanks === 1 ? " está" : "s estão"} em branco — não vão corresponder a nada.`,
        });
      }
    }
  }
    ...
    case "start": {
      ...
      if (!cfg.next_node_key) {
        issues.push({
          ...
          field: "next_node_key",
          message: "O nó inicial precisa apontar para um próximo nó.",
        });
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó inicial aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "send_message": {
      ...
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "O nó de envio de mensagem precisa apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó de envio de mensagem aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "send_media": {
      ...
      if (
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "media_type",
          message: "O nó de envio de mídia precisa de um tipo de mídia (image, video ou document).",
        });
      }
      if (!cfg.media_url?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "media_url",
          message: "O nó de envio de mídia precisa de um arquivo (faça o upload antes de ativar).",
        });
      }
      ...
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "O nó de envio de mídia precisa apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó de envio de mídia aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "send_buttons": {
      ...
      if (!cfg.text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "text",
          message: "O nó de botões precisa de um corpo de texto.",
        });
      }
    ...
    case "send_list": {
      ...
      if (!cfg.text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "text",
          message: "O nó de lista precisa de um corpo de texto.",
        });
      }
      if (!cfg.button_label?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "button_label",
          message: "O nó de lista precisa de um rótulo de botão (o texto de toque para expandir).",
        });
      }
      ...
      if (totalRows > INTERACTIVE_LIMITS.maxListRowsTotal) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "sections",
          message: `O nó de lista permite no máximo ${INTERACTIVE_LIMITS.maxListRowsTotal} linhas no total entre as seções.`,
        });
      }
      const seenIds = new Set<string>();
        ...
        rows.forEach((row, ri) => {
          ...
          if (!row.reply_id?.trim()) {
            issues.push({
              ...
              node_key: node.node_key,
              field: `${field}.reply_id`,
              message: `A linha ${ri + 1} da seção ${si + 1} precisa de um reply id.`,
            });
    ...
    case "collect_input": {
      ...
      if (!cfg.prompt_text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "prompt_text",
          message: "O nó de coleta de resposta precisa de uma pergunta para enviar ao cliente.",
        });
      }
      if (!cfg.var_key?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: "O nó de coleta de resposta precisa de um var_key para armazenar a resposta.",
        });
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: `O var_key "${cfg.var_key}" deve conter apenas letras, números e sublinhado, começando por letra ou sublinhado.`,
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "O nó de coleta de resposta precisa apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó de coleta de resposta aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "condition": {
      ...
      if (!cfg.subject || !["var", "tag", "contact_field"].includes(cfg.subject)) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "subject",
          message: "A condição precisa de um sujeito (var / tag / contact_field).",
        });
      }
      ...
      if (
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "value",
          message: `O operador "${cfg.operator}" normalmente espera um valor de comparação — um valor vazio só corresponderá a sujeitos vazios.`,
        });
      }
      for (const branch of ["true_next", "false_next"] as const) {
        ...
        if (!key) {
          issues.push({
            ...
            node_key: node.node_key,
            field: branch,
            message: `A condição precisa de um nó para o ramo "${branch === "true_next" ? "true" : "false"}".`,
          });
    ...
    case "set_tag": {
      ...
      if (!cfg.mode || !["add", "remove"].includes(cfg.mode)) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "mode",
          message: "O nó de tag precisa de um modo (add ou remove).",
        });
      }
      if (!cfg.tag_id) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "tag_id",
          message: "O nó de tag precisa de uma tag para aplicar.",
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "O nó de tag precisa apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó de tag aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
