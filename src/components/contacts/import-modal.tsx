  ...
  async function handleImport() {
    ...
    try {
      ...
      try {
        ...
        toast.warning('Contatos importados, mas algumas atribuições de etiqueta falharam.');
      }
      ...
      if (skippedNames.length > 0) {
        ...
        toast.info(
          `Etiquetas desconhecidas ignoradas (crie-as primeiro nas Configurações): ${sample}${more}`
        );
      }
      if (skipped > 0) {
      ...
      if (failed > 0) {
        toast.error(
          `${failed} contato${failed !== 1 ? 's' : ''} não pôde ser importado`
        );
      }
    } catch (err: unknown) {
